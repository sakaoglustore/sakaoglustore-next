...
export default function Signup() {
  ...
  const handleSubmit = async e => {
    ...
    if (!passwordRegex.test(form.password)) {
      return setMessage('Şifre geçersiz: 1 büyük, 1 küçük harf, sembol ve 6-16 karakter olmalı.');
    }

    const res = await fetch('http://13.53.182.174:5000/api/auth/signup', {
      method: 'POST',
    ...
    if (res.status === 201) {
      const loginRes = await fetch('http://13.53.182.174:5000/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
